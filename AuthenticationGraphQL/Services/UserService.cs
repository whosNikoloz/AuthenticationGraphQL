...
namespace AuthenticationGraphQL.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<IActionResult> ChangeEmailRequestAsync(string email ,int userid)
        {
            ...
            if (_db.Users.Any(u => u.Email == email && u.OAuthProvider == null))
            {
                return new NotFoundObjectResult("ასეთი ანგარიში უკვე რეგისტრირებულია");
            }
        ...
        public async Task<IActionResult> ChangeGeneralInfoAsync(ChangeGeneralRequest request , int userId, string JWTRole)
        {
            ...
            {
                return new NotFoundObjectResult("Username already exists in the database.");
            }
            ...
            if (userId != user.UserId)
            {
                if (JWTRole != "admin")
                {
                    return new NotFoundObjectResult("Authorize invalid");
                }
            }
        ...
        public async Task<IActionResult> ReLoginAsync(string password, string userId)
        {
        ...
        public async Task<IActionResult> UploadUserProfileImageAsync(UploadImageRequest imageRequest, string userId, string JWTRole)
        {
            ...
            {
                await _db.SaveChangesAsync();
                return new OkObjectResult("Updated Succes");
            }
            catch (Exception ex)
            {
                Console.WriteLine("Exception occurred during SaveChangesAsync: " + ex.Message);
